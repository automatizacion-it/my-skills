...
const intents = [
  ...
  },
  {
    ...
    description: "Poner una canción o artista (Ej: 'Pon Bad Bunny', 'Pon música de Queen')",
    match: (c) => {
      const triggers = ['pon ', 'reproduce ', 'busca ', 'quiero escuchar ', 'ponme '];
      return triggers.some(t => c.startsWith(t));
    },
    action: (comando) => {
      ...
      // Limpiar preposiciones y palabras genéricas al inicio
      query = query
        .replace(/^(música de |música del |música a |música |algo de |canciones de |canción de |un poco de )/i, '')
        .replace(/\b(por favor)\b/gi, '')
        .trim();
  ...
  },
  {
    ...
    match: (c) => c === 'pon música' || c === 'reproducir música' || c.includes('pon algo de música'),
    action: () => {
  ...
  },
  {
    ...
    match: (c) => c.includes('anterior') || c.includes('la de antes'),
